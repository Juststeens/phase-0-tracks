...
# 1. the main hash structure uses a hash rocket and the key is a string.
#  the second structure uses a colon to seperate the key from the value
#  and makes the key a symbol

# 2. require_relative allows us to link a file to another file
#  require performs the same function but can also be used for methods
#  and modules.
#
# 3. you can use a .each loop to allow you to call both the key and value
#  at different times. you could also use a while loop with each.
#
# 4. the variables were being used to call parameters onto methods
#
# 5. Iterating through a nested data structure.
#
#